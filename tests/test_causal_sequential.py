...
class TestCausalSequentialEngineSQLite:
    ...
    async def vector_store(self):
        ...
        store = QdrantStore(
            host="localhost",
            port=6333,
            collection_name="test_causal_sqlite",
            vector_size=768
        )
    ...
    async def test_create_sequential_chain(self, engine, graph_store):
        ...
        for idx, (mem_id, text) in enumerate(memories):
        ...
        print(f"  ✓ Created sequential chain with FOLLOWS edges")
    ...
    async def test_detect_conversation_threads(self, engine, graph_store):
        ...
        threads = await engine.detect_conversation_threads(
            memory_ids=memory_ids,
            max_gap_seconds=3600  # 1 hour max gap
        )
        ...
        print(f"  ✓ Thread detection works (grouped by temporal proximity)")
    ...
    async def test_create_conversation_threads(self, engine, graph_store):
        ...
        print(f"  ✓ Created conversation thread edges")
    ...
    async def test_create_version_history(self, engine, graph_store):
        ...
        for mem_id, text, version in versions:
            await graph_store.add_node(mem_id, NodeType.MEMORY, {"text": text})
        ...
        print(f"  ✓ Created version history chain")
    ...
    async def test_get_version_chain(self, engine, graph_store):
        ...
        for i in range(len(versions) - 1):
            await engine.create_version_history(
                memory_id=versions[i + 1],
                previous_version_id=versions[i],
                version_number=i + 2
            )
        ...
        print(f"  ✓ Retrieved version chains correctly")
    ...
    async def test_detect_prerequisites(self, engine, graph_store, vector_store, embedder):
        ...
        memories = [
            ...
            ("prereq-adv", "Python decorators modify function behavior using closures and functions"),
        ]
        ...
        prerequisites = await engine.detect_prerequisites(
            memory_id="prereq-adv",
            candidate_ids=["prereq-basic", "prereq-inter"]
        )
    ...
    async def test_create_prerequisite_edges(self, engine, graph_store, vector_store, embedder):
        ...
        print(f"  ✓ Prerequisite analysis completed")
    ...
    async def test_get_knowledge_path(self, engine, graph_store):
        ...
        for i in range(len(concepts) - 1):
            await engine.create_prerequisite_edges(
                memory_id=concepts[i + 1],
                candidate_ids=[concepts[i]],
                max_prerequisites=1
            )
        ...
        if path:
            ...
            print(f"  No path found (expected if edges don't exist)")
        
        print(f"  ✓ Knowledge path search completed")
...
class TestCausalSequentialEngineNeo4j:
    ...
    async def vector_store(self):
        ...
        store = QdrantStore(
            host="localhost",
            port=6333,
            collection_name="test_causal_neo4j",
            vector_size=768
        )
        await store.initialize()
        yield store
    ...
    async def graph_store(self):
        ...
        async with driver.session() as session:
            await session.run(
                "MATCH (n:Node) WHERE n.id STARTS WITH 'neo-causal-' DETACH DELETE n"
            )
    ...
    async def test_sequential_chain_neo4j(self, engine, graph_store):
        ...
        print(f"  ✓ Created FOLLOWS relationships in Neo4j")
    ...
    async def test_version_history_neo4j(self, engine, graph_store):
        ...
        edge_id = await engine.create_version_history(
            memory_id="neo-causal-v2",
            previous_version_id="neo-causal-v1",
            version_number=2
        )
        ...
        assert edge_id != ""
        print(f"  ✓ Created UPDATES relationship in Neo4j")

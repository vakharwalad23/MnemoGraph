...
class TestQdrantStore:
    ...
    async def store(self):
        ...
        store = QdrantStore(
            host="localhost", port=6333, collection_name="test_memories", vector_size=768
        )
    ...
    async def test_search_with_threshold(self, store, sample_embedding):
        ...
        results = await store.search_similar(sample_embedding, limit=10, score_threshold=0.99)
    ...
    async def test_search_with_filter(self, store, sample_embedding):
        ...
        await store.upsert_memory(
            "mem-1", sample_embedding, {"text": "Memory 1", "status": "active"}
        )
        await store.upsert_memory("mem-2", [0.2] * 768, {"text": "Memory 2", "status": "forgotten"})

        # Search only for active memories
        results = await store.search_similar(
            sample_embedding, limit=10, filter_dict={"status": "active"}
        )
    ...
    async def test_upsert_updates_existing(self, store, sample_embedding):
        ...
        # Initial insert
        await store.upsert_memory(memory_id, sample_embedding, {"text": "Original text"})

        # Update with new metadata
        await store.upsert_memory(memory_id, sample_embedding, {"text": "Updated text"})

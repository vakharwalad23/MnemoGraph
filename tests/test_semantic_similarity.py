...
class TestSemanticSimilarityEngineSQLite:
    ...
    async def vector_store(self):
        ...
        store = QdrantStore(
            host="localhost", port=6333, collection_name="test_semantic_sim_sqlite", vector_size=768
        )
    ...
    async def test_compute_batch_similarity_real(self, real_embeddings):
        ...
        similarities = SemanticSimilarityEngine.compute_batch_similarity(query, embeddings)
    ...
    async def test_infer_relationships_real_embeddings(
        ...
        similar = await engine.infer_relationships(
            memory_id="mem-python-1", embedding=real_embeddings["python1"], create_edges=True
        )
        ...
        neighbors = await graph_store.get_neighbors(
            "mem-python-1", edge_types=[RelationshipType.SIMILAR_TO], direction="outgoing"
        )
    ...
    async def test_threshold_filtering_real(
        ...
        await vector_store.upsert_memory(
            "mem-1", real_embeddings["python1"], {"text": "Python programming"}
        )
        await vector_store.upsert_memory(
            "mem-2", real_embeddings["python2"], {"text": "Python development"}
        )
        await vector_store.upsert_memory(
            "mem-3", real_embeddings["cooking"], {"text": "Cooking recipes"}
        )
        ...
        similar = await engine.infer_relationships(
            memory_id="mem-1", embedding=real_embeddings["python1"], create_edges=True
        )
    ...
    @pytest.mark.asyncio
    async def test_batch_infer_real(self, engine, vector_store, graph_store, real_embeddings):
        ...
        results = await engine.batch_infer_relationships(memories=memories, create_edges=True)
    ...
    async def test_find_clusters_real(self, engine, vector_store, real_embeddings):
        ...
        clusters = await engine.find_clusters(
            memory_ids=list(memory_map.keys()), min_cluster_size=2
        )
...
class TestSemanticSimilarityEngineNeo4j:
    ...
    async def vector_store(self):
        """Create a Qdrant vector store."""
        store = QdrantStore(
            host="localhost", port=6333, collection_name="test_semantic_sim_neo4j", vector_size=768
        )
        await store.initialize()
        yield store
    ...
    async def test_infer_relationships_neo4j(
        ...
        # Infer relationships
        similar = await engine.infer_relationships(
            memory_id="neo-mem-python-1", embedding=real_embeddings["python1"], create_edges=True
        )
        ...
        neighbors = await graph_store.get_neighbors(
            "neo-mem-python-1", edge_types=[RelationshipType.SIMILAR_TO], direction="outgoing"
        )
    ...
    @pytest.mark.asyncio
    async def test_batch_infer_neo4j(self, engine, vector_store, graph_store, real_embeddings):
    ...
    @pytest.mark.asyncio
    async def test_edge_metadata_neo4j(self, engine, vector_store, graph_store, real_embeddings):
        """Test that Neo4j edges include proper metadata."""
        await vector_store.upsert_memory(
            "neo-mem-1", real_embeddings["python1"], {"text": "Test 1"}
        )
        await vector_store.upsert_memory(
            "neo-mem-2", real_embeddings["python2"], {"text": "Test 2"}
        )
        ...
        await engine.infer_relationships(
            memory_id="neo-mem-1", embedding=real_embeddings["python1"], create_edges=True
        )

        # Check edges
        neighbors = await graph_store.get_neighbors(
            "neo-mem-1", edge_types=[RelationshipType.SIMILAR_TO], direction="outgoing"
        )
...
class TestSemanticSimilarityEngineConfiguration:
    ...
    async def test_max_similar_memories_limit(self, tmp_path):
        ...
        engine = SemanticSimilarityEngine(
            vector_store=vector_store, graph_store=graph_store, max_similar_memories=3
        )

...
from src.models import MemoryStatus, NodeType, RelationshipType
...
class TestTemporalRelationshipEngineSQLite:
    ...
    async def vector_store(self):
        ...
        store = QdrantStore(
            host="localhost", port=6333, collection_name="test_temporal_sqlite", vector_size=768
        )
    ...
    async def test_detect_expiring_memories_fresh(self, engine):
        ...
        decay_info = await engine.detect_expiring_memories(
            memory_id="mem-1", created_at=created, last_accessed=now, access_count=5
        )
    ...
    async def test_consolidation_effect(self, engine):
        ...
        decay_low = await engine.detect_expiring_memories(
            memory_id="mem-1", created_at=created, last_accessed=created, access_count=1
        )
        ...
        print("  ✓ Frequently accessed memories resist forgetting!")
    ...
    async def test_detect_updates_real_embeddings(
        ...
        await graph_store.add_node("mem-old-version", NodeType.MEMORY, {"text": "Old Python info"})
        ...
        await graph_store.add_node(
            "mem-new-version", NodeType.MEMORY, {"text": "Updated Python info"}
        )
        ...
        if updates:
            for update in updates:
                print(
                    f"  - {update['id']}: similarity={update['score']:.4f}, "
                    f"{update['days_older']:.1f} days older"
                )

            # Verify UPDATE edge was created
            neighbors = await graph_store.get_neighbors(
                "mem-new-version", edge_types=[RelationshipType.UPDATES], direction="outgoing"
            )
    ...
    async def test_create_temporal_sequence(self, engine, graph_store):
        ...
        for i in range(5):
            mem_id = f"conv-msg-{i}"
            await graph_store.add_node(
                mem_id, NodeType.MEMORY, {"text": f"Message {i} in conversation"}
            )
            memory_ids.append(mem_id)

        # Create sequence
        edges_created = await engine.create_temporal_sequence(
            memory_ids=memory_ids, sequence_type="conversation"
        )
        ...
        # Verify edges exist
        neighbors = await graph_store.get_neighbors(
            "conv-msg-0", edge_types=[RelationshipType.FOLLOWS], direction="outgoing"
        )

        assert len(neighbors) == 1
        assert neighbors[0]["node"].id == "conv-msg-1"
        print("  ✓ Message 0 → Message 1 (FOLLOWS)")
    ...
    async def test_apply_decay_to_node(self, engine, graph_store):
        ...
        # Create node
        await graph_store.add_node(
            "mem-decay-test", NodeType.MEMORY, {"text": "Test memory for decay"}
        )

        # Calculate decay
        decay_info = await engine.detect_expiring_memories(
            memory_id="mem-decay-test", created_at=created, last_accessed=created, access_count=0
        )
        ...
        print("  ✓ NEW → ACTIVE → EXPIRING_SOON → FORGOTTEN")
    ...
    async def test_run_decay_cycle(self, engine, graph_store):
        ...
        memory_ids = []
        for mem_id, _created, text in memories:
        ...
        print("  ✓ Like how the brain consolidates memories during sleep!")
    ...
    async def test_temporal_clusters(self, engine, graph_store):
        ...
        _cluster1_base = now - timedelta(hours=10)

        # Cluster 2: Memories from 50 hours ago
        _cluster2_base = now - timedelta(hours=50)
        ...
        for i in range(3):
            ...
            await graph_store.add_node(mem_id, NodeType.MEMORY, {"text": f"Morning event {i}"})
            memory_ids.append(mem_id)

        # Create cluster 2 (2 memories close in time)
        for i in range(2):
            mem_id = f"yesterday-event-{i}"
            await graph_store.add_node(mem_id, NodeType.MEMORY, {"text": f"Yesterday event {i}"})
            memory_ids.append(mem_id)

        # Find temporal clusters
        clusters = await engine.detect_temporal_clusters(
            memory_ids=memory_ids, time_window_hours=24
        )
        ...
        print("  ✓ Brain groups memories from same context/time!")
    ...
    async def test_calculate_temporal_relevance(self, engine):
        ...
        rel_recent = engine.calculate_temporal_relevance(
            created_at=recent_created, last_accessed=now, access_count=10
        )

        # Old, rarely accessed (low relevance)
        old_created = now - timedelta(days=90)
        rel_old = engine.calculate_temporal_relevance(
            created_at=old_created, last_accessed=old_created, access_count=1
        )
        ...
        print("  ✓ Recent and frequently used memories are more accessible!")
...
class TestTemporalRelationshipEngineNeo4j:
    ...
    async def vector_store(self):
        """Create a Qdrant vector store."""
        store = QdrantStore(
            host="localhost", port=6333, collection_name="test_temporal_neo4j", vector_size=768
        )
        await store.initialize()
        yield store
    ...
    async def graph_store(self):
        ...
        async with driver.session() as session:
            await session.run("MATCH (n:Node) WHERE n.id STARTS WITH 'neo-temp-' DETACH DELETE n")
    ...
    @pytest.mark.asyncio
    async def test_detect_updates_neo4j(self, engine, vector_store, graph_store, real_embeddings):
        ...
        await graph_store.add_node("neo-temp-python-v1", NodeType.MEMORY, {"text": "Python 3.8"})
        ...
        await graph_store.add_node("neo-temp-python-v2", NodeType.MEMORY, {"text": "Python 3.11"})
        ...
        if updates:
            ...
            # Verify Neo4j edge
            neighbors = await graph_store.get_neighbors(
                "neo-temp-python-v2", edge_types=[RelationshipType.UPDATES], direction="outgoing"
            )
            assert len(neighbors) >= 1
            print("  ✓ Created UPDATE relationship in Neo4j")
    ...
    async def test_temporal_sequence_neo4j(self, engine, graph_store):
        ...
        for i in range(4):
            ...
            await graph_store.add_node(mem_id, NodeType.MEMORY, {"text": f"Message {i}"})
            memory_ids.append(mem_id)
        ...
        # Verify with Neo4j
        neighbors = await graph_store.get_neighbors(
            "neo-temp-conv-0", edge_types=[RelationshipType.FOLLOWS], direction="outgoing"
        )

        assert len(neighbors) == 1
        assert neighbors[0]["node"].id == "neo-temp-conv-1"
        print("  ✓ Sequential relationships verified in Neo4j graph")
    ...
    async def test_decay_cycle_neo4j(self, engine, graph_store):
        ...
        memory_ids = []
        for mem_id, _created in memories:
        ...
        print("  ✓ Memory consolidation in Neo4j graph")

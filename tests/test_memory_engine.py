...
class TestMemoryEngineSQLite:
    ...
    async def vector_store(self):
        ...
        store = QdrantStore(
            host="localhost", port=6333, collection_name="test_memory_engine", vector_size=768
        )
    ...
    async def test_add_memory_with_relationships(self, engine):
        ...
        # Add first memory
        result1 = await engine.add_memory(text="Python is a programming language")

        # Add related memory
        result2 = await engine.add_memory(text="Python 3.11 introduces performance improvements")
    ...
    async def test_query_memories(self, engine):
        ...
        results = await engine.query_memories(query="Python programming", limit=5)
        ...
        python_results = sum(
            1 for r in results if "python" in r["metadata"].get("text", "").lower()
        )
    ...
    async def test_query_with_relationships(self, engine):
        ...
        # Add memories
        await engine.add_memory("Python programming basics")
        await engine.add_memory("Advanced Python techniques")

        # Query with relationships
        results = await engine.query_memories(query="Python", limit=5, include_relationships=True)
        ...
        has_relationships = any(r.get("relationships", {}).get("count", 0) > 0 for r in results)
    ...
    async def test_get_memory(self, engine):
        ...
        # Add memory
        result = await engine.add_memory(text="Test memory content", metadata={"category": "test"})
    ...
    async def test_get_memory_with_relationships(self, engine):
        ...
        await engine.add_memory("First memory")
        mem2 = await engine.add_memory("Related second memory")

        # Get with relationships
        memory = await engine.get_memory(mem2["memory_id"], include_relationships=True)
    ...
    async def test_update_memory(self, engine):
        ...
        update_result = await engine.update_memory(
            memory_id=memory_id, text="Updated text", metadata={"updated": True}
        )
    ...
    async def test_add_conversation(self, engine):
        ...
        result = await engine.add_conversation(
            messages=messages, metadata={"session": "test-session"}
        )
    ...
    async def test_memory_without_auto_infer(self, engine):
        ...
        result = await engine.add_memory(text="Standalone memory", auto_infer_relationships=False)
...
class TestMemoryEngineNeo4j:
    ...
    async def vector_store(self):
        """Create Qdrant vector store."""
        store = QdrantStore(
            host="localhost", port=6333, collection_name="test_memory_engine_neo4j", vector_size=768
        )
        await store.initialize()
        yield store
    ...
    async def graph_store(self):
        ...
        await store.driver.execute_query("MATCH (n) DETACH DELETE n")
        await store.close()
    ...
    async def test_add_memory_neo4j(self, engine):
        ...
        result = await engine.add_memory(text="Neo4j test memory", metadata={"source": "test"})

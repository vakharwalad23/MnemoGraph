...
class TestRelationshipOrchestrator:
    ...
    @pytest.mark.asyncio
    async def test_process_new_memory(
    ...
    @pytest.mark.asyncio
    async def test_batch_process_memories(
        self, orchestrator, graph_store, vector_store, embedder
    ):
        ...
        # Batch process
        stats = await orchestrator.batch_process_memories(
    ...
    @pytest.mark.asyncio
    async def test_create_conversation_thread(
        self, orchestrator, graph_store
    ):
        ...
        result = await orchestrator.create_conversation_thread(
            ...
            thread_metadata={"conversation_id": "conv-123"}
        )
        ...
        print(f"  ✓ Conversation thread created successfully")
    
    @pytest.mark.asyncio
    async def test_create_document_hierarchy(
        self, orchestrator, graph_store
    ):
        """Test creating document hierarchy."""
        # Create document and chunks
        doc_id = "doc-1"
        await graph_store.add_node(doc_id, NodeType.DOCUMENT, {
        ...
        for i in range(5):
            chunk_id = f"chunk-{i}"
            await graph_store.add_node(chunk_id, NodeType.CHUNK, {
        ...
        print(f"  ✓ Document hierarchy created successfully")
...
class TestRelationshipOrchestratorNeo4j:
    ...
    async def vector_store(self):
        ...
        store = QdrantStore(
            host="localhost",
            port=6333,
            collection_name="test_orchestrator_neo4j",
            vector_size=768
        )
        await store.initialize()
        yield store
    ...
    @pytest.mark.asyncio
    async def test_create_conversation_thread_neo4j(
        self, orchestrator, graph_store
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_document_hierarchy_neo4j(
        self, orchestrator, graph_store
    ):
...
class TestOrchestratorConfiguration:
    ...
    async def test_custom_config(self, tmp_path):
        ...
        assert orchestrator.hierarchical_engine.min_cluster_size == 3  # default

...
class TestHierarchicalRelationshipEngineSQLite:
    ...
    async def vector_store(self):
        ...
        store = QdrantStore(
            host="localhost",
            port=6333,
            collection_name="test_hierarchical_sqlite",
            vector_size=768
        )
    ...
    async def test_create_parent_child_relationship(self, engine, graph_store):
        ...
        neighbors = await graph_store.get_neighbors(
            "doc-1",
            edge_types=[RelationshipType.PARENT_OF],
            direction="outgoing"
        )
        
        assert len(neighbors) == 1
        assert neighbors[0]["node"].id == "chunk-1"
        print(f"  ✓ Verified: doc-1 is parent of chunk-1")
    ...
    async def test_create_topic_clusters_real_embeddings(
        ...
        result = await engine.create_topic_clusters(
            memory_ids=memory_ids,
            create_topic_nodes=True
        )
    ...
    async def test_detect_abstraction_levels(
        ...
        memories = [
            ...
            ("mem-specific", real_embeddings["python_specific"], 
        ...
        abstractions = await engine.detect_abstraction_levels(
            memory_ids=["mem-general", "mem-specific"],
            create_edges=True
        )
        ...
        for specific, general, confidence in abstractions:
            ...
            print(f"  ✓ Specific details abstract to general concept")
...
class TestHierarchicalRelationshipEngineNeo4j:
    ...
    async def vector_store(self):
        ...
        store = QdrantStore(
            host="localhost",
            port=6333,
            collection_name="test_hierarchical_neo4j",
            vector_size=768
        )
        await store.initialize()
        yield store
    ...
    async def test_chunk_document_neo4j(self, engine, graph_store):
        ...
        print(f"  ✓ All chunks linked in Neo4j graph")
    
    @pytest.mark.asyncio
    async def test_topic_clusters_neo4j(
        self, engine, vector_store, graph_store, real_embeddings
    ):
        ...
        assert result["num_clusters"] >= 1
        print(f"  ✓ Created topic nodes in Neo4j")

...
class TestSQLiteGraphStore:
    ...
    async def test_get_neighbors_filtered_by_type(self, store):
        ...
        neighbors = await store.get_neighbors(
            "node-1", edge_types=[RelationshipType.SIMILAR_TO], direction="outgoing"
        )
    ...
    async def test_update_edge_weight(self, store):
        ...
        edge_id = await store.add_edge("node-1", "node-2", RelationshipType.SIMILAR_TO, 0.5)
...
class TestNeo4jGraphStore:
    ...
    async def test_get_neighbors_filtered_by_type(self, store):
        ...
        neighbors = await store.get_neighbors(
            "neo-node-1", edge_types=[RelationshipType.SIMILAR_TO], direction="outgoing"
        )
...
class TestGraphStoreFactory:
    ...
    def test_create_neo4j_store(self):
        ...
        store = create_graph_store(
            backend="neo4j", uri="bolt://localhost:7687", user="neo4j", password="test123"
        )

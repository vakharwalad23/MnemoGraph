...
class EntityCooccurrenceEngine:
                    ...
                    if chunk.root.pos_ in ["NOUN", "PROPN"]:
                        ...
                        for token in chunk:
                            if (
                                ...
                                and len(token.text) >= self.min_entity_length
                            ):
    ...
    async def detect_cooccurrence(
        self, memory_id: str, entities: set[str], create_edges: bool = True
    ) -> list[dict[str, Any]]:
        """
    ...
    async def build_entity_index(self, memory_ids: list[str]) -> dict[str, list[str]]:
        """
    ...
    async def create_cooccurrence_edges(self, memory_ids: list[str]) -> dict[str, Any]:
        """
    ...
    async def find_memories_by_entity(self, entity: str, memory_ids: list[str]) -> list[str]:
        """
    ...
    def calculate_entity_importance(self, entity_stats: dict[str, int]) -> dict[str, float]:
        """
    ...
    async def get_entity_graph(self, memory_ids: list[str]) -> dict[str, Any]:
        """
        ...
        for entity1, related in entity_cooccurrence.items():
            for entity2, count in related.items():
                if entity1 < entity2:  # Avoid duplicates
                    edges.append({"source": entity1, "target": entity2, "weight": count})
    ...
    async def find_entity_clusters(
        self, memory_ids: list[str], min_cluster_size: int = 3
    ) -> list[set[str]]:
        """

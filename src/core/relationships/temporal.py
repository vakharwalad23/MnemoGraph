...
from src.models import RelationshipType, MemoryStatus
...
class TemporalRelationshipEngine:
    ...
    async def detect_updates(
        ...
        similar_memories = await self.vector_store.search_similar(
            ...
            limit=20,
            score_threshold=self.update_similarity_threshold
        )
        ...
        for mem in similar_memories:
            ...
            try:
                mem_created_dt = datetime.fromisoformat(mem_created).replace(tzinfo=timezone.utc)
    ...
    async def create_temporal_sequence(
        self,
        memory_ids: List[str],
        sequence_type: str = "conversation"
    ) -> int:
        """

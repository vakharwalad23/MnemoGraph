...
from src.models import RelationshipType, NodeType
...
class HierarchicalRelationshipEngine:
    ...
    async def create_topic_clusters(
        ...
        for mem_id, label in zip(valid_ids, cluster_labels):
    ...
    async def detect_abstraction_levels(
        ...
        for i, mem1 in enumerate(memories):
            for j, mem2 in enumerate(memories):
                ...
                length_ratio = min(mem1["length"], mem2["length"]) / max(mem1["length"], mem2["length"])

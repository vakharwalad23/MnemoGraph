...
class CausalSequentialEngine:
    ...
    async def create_sequential_chain(
        ...
        """
        if len(memory_ids) < 2:
            return {"edges_created": 0, "chain_length": len(memory_ids), "chain_type": chain_type}
    ...
    async def detect_conversation_threads(
        self, memory_ids: list[str], max_gap_seconds: int | None = None
    ) -> list[list[str]]:
        """
    ...
    async def create_conversation_threads(
        ...
        for thread_idx, thread in enumerate(threads):
            result = await self.create_sequential_chain(
                memory_ids=thread, chain_type="conversation", metadata={"thread_id": thread_idx}
            )
            total_edges += result["edges_created"]
            thread_stats.append(
                {"thread_id": thread_idx, "length": len(thread), "edges": result["edges_created"]}
            )
    ...
    async def get_version_chain(
        self, memory_id: str, direction: str = "forward"
    ) -> list[dict[str, Any]]:
        """
        ...
        while current_id and current_id not in visited:
            ...
            versions.append({"id": current_id, "created_at": node.created_at, "data": node.data})

            # Get next/previous version
            if direction == "forward":
                neighbors = await self.graph_store.get_neighbors(
                    current_id, edge_types=[RelationshipType.UPDATES], direction="outgoing"
                )
            else:
                neighbors = await self.graph_store.get_neighbors(
                    current_id, edge_types=[RelationshipType.UPDATES], direction="incoming"
                )
        ...
        if direction == "forward":
            ...
            start_idx = next((i for i, v in enumerate(versions) if v["id"] == memory_id), 0)
            return versions[start_idx:]
        else:
            # Return up to and including starting memory
            start_idx = next(
                (i for i, v in enumerate(versions) if v["id"] == memory_id), len(versions) - 1
            )
            return versions[: start_idx + 1]

    async def detect_prerequisites(
        self, memory_id: str, candidate_ids: list[str], max_candidates: int = 5
    ...
    async def create_prerequisite_edges(
        self, memory_id: str, candidate_ids: list[str], max_prerequisites: int = 3
    ) -> dict[str, Any]:
        """
        ...
        prerequisites = await self.detect_prerequisites(
            memory_id, candidate_ids, max_candidates=max_prerequisites
        )
        ...
        for prereq_id, relevance in prerequisites:
            edge_id = await self.graph_store.add_edge(
                ...
                weight=relevance,
                metadata={"relevance_score": relevance, "relationship_type": "prerequisite"},
            )

            edges_created += 1
            prerequisite_info.append(
                {"prerequisite_id": prereq_id, "relevance": relevance, "edge_id": edge_id}
            )
    ...
    async def detect_topic_shifts(self, memory_ids: list[str]) -> list[int]:
        """
        ...
        for i in range(len(memory_ids) - 1):
            ...
            results = await self.vector_store.search_similar(
                query_vector=curr_memory_data["vector"], limit=10
            )
    ...
    async def analyze_sequence(self, memory_ids: list[str]) -> dict[str, Any]:
        """

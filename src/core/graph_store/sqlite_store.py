...
class SQLiteGraphStore(GraphStore):
    ...
    async def initialize(self) -> None:
        ...
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_edges_type ON edges(type)")
        await conn.execute(
            ...
            return [
                {
                    ...
                    "edge_id": row["edge_id"],
                    "edge_metadata": (
                        json.loads(row["edge_metadata"]) if row["edge_metadata"] else None
                    ),
                }
                for row in rows
            ]
    ...
    async def update_node_status(self, node_id: str, status: MemoryStatus) -> None:
        ...
        await conn.execute("UPDATE nodes SET status = ? WHERE id = ?", (status.value, node_id))
    ...
    async def update_node_access(
        ...
        await conn.execute(
            ...
            WHERE id = ?
        """,
            (access_count, last_accessed.replace(tzinfo=UTC).isoformat(), node_id),
        )
    ...
    async def update_edge_weight(self, edge_id: str, weight: float) -> None:
        ...
        await conn.execute("UPDATE edges SET weight = ? WHERE id = ?", (weight, edge_id))
    ...
    async def find_path(self, start_id: str, end_id: str, max_depth: int = 3) -> list[str] | None:
        ...
        while queue:
            ...
            async with conn.execute(
                "SELECT target FROM edges WHERE source = ?", (current_id,)
            ) as cursor:
                rows = await cursor.fetchall()
    ...
    async def delete_node(self, node_id: str) -> None:
        ...
        await conn.execute("DELETE FROM edges WHERE source = ? OR target = ?", (node_id, node_id))

...
class RelationshipOrchestrator:
    ...
    async def process_new_memory(
        ...
        if context_memory_ids:
            tasks.append(self._run_hierarchical_inference(
                memory_id, context_memory_ids, stats
            ))
        
        # 4. Entity co-occurrence (needs context)
        if context_memory_ids:
            tasks.append(self._run_cooccurrence_inference(
                memory_id, text, context_memory_ids, stats
            ))
        
        # 5. Causal/sequential (needs context)
        if context_memory_ids:
            tasks.append(self._run_causal_inference(
                memory_id, context_memory_ids, stats
            ))
    ...
    async def _run_hierarchical_inference(
        ...
        try:
            ...
            result = await self.hierarchical_engine.create_topic_clusters(
                memory_ids=all_ids
            )
    ...
    async def _run_causal_inference(
        ...
        try:
            ...
            result = await self.causal_engine.create_prerequisite_edges(
                memory_id=memory_id,
                candidate_ids=context_memory_ids,
                max_prerequisites=3
            )
    ...
    async def create_document_hierarchy(
        ...
        """
        logger.info(
            f"Creating document hierarchy: {document_id} with {len(chunk_ids)} chunks"
        )

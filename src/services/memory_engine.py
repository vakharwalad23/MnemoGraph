...
from src.models import NodeType, Memory
...
class MemoryEngine:
    ...
    async def add_memory(
        ...
        auto_infer_relationships: Optional[bool] = None,
        ...
        if auto_infer_relationships:
            ...
            context_ids = [
                mem["id"] for mem in similar_memories 
                if mem["id"] != memory_id
            ][:context_window_size]
        ...
        if include_relationships:
            for result in results:
                ...
                result["relationships"] = {
                    "count": len(neighbors),
                    "types": list(set(n.get("edge_type") for n in neighbors))
                }
    ...
    async def delete_memory(
        ...
        logger.info(
            ...
            f"(removed {relationship_count} relationships)"
        )
    ...
    async def add_document(
        ...
        if self.config.relationships.auto_infer_on_add:
            ...
            total_relationships = (
                hierarchy_result["edges_created"] + 
                batch_result["total_relationships"]
            )
        else:
